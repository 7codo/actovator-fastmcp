...
from app.constants import SERENA_FILE_ENCODING
from app.utils.text_utils import search_files
from app.tools.tools_base import SUCCESS_RESULT, EditedFileContext, Tool
from app.utils.file_system import scan_directory
...
class ReadFileTool(Tool):
    ...
    def apply(
        ...
        """
        self.validate_relative_path(relative_path, require_not_ignored=True)
...
class ListDirTool(Tool):
    ...
    def apply(
        ...
        if not self.relative_path_exists(relative_path):
            error_info = {
...
class FindFileTool(Tool):
    ...
    def apply(self, file_mask: str, relative_path: str) -> str:
        ...
        def is_ignored_file(abs_path: str) -> bool:
            if self.is_ignored_path(abs_path):
                return True
...
class ReplaceRegexTool(Tool):
    ...
    def replace_regex(
        ...
        with EditedFileContext(relative_path) as context:
            original_content = context.get_original_content()
            updated_content, n = re.subn(
                regex, repl, original_content, flags=re.DOTALL | re.MULTILINE
            )
            if n == 0:
...
class ReplaceLinesTool(Tool):
    ...
    def apply(
        ...
        if not content.endswith("\n"):
            content += "\n"
        result = self.get_tool(DeleteLinesTool).apply(
            relative_path, start_line, end_line
        )
        if result != SUCCESS_RESULT:
            return result
        self.get_tool(InsertAtLineTool).apply(relative_path, start_line, content)
        return SUCCESS_RESULT
...
class SearchForPatternTool(Tool):
    ...
    def apply(
        ...
        if restrict_search_to_code_files:
            matches = self.search_source_files_for_pattern(
