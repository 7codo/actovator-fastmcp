...
from app.tools.tools_base import Tool, SUCCESS_RESULT
from app.solidlsp.ls_types import SymbolKind
    ...
    def apply(self, relative_path: str, max_answer_chars: int = -1) -> str:
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(
                f"File or directory {relative_path} does not exist in the project."
            )
        if os.path.isdir(file_path):
            raise ValueError(
                f"Expected a file path, but got a directory path: {relative_path}. "
            )
...
class FindSymbolTool(Tool):
    ...
    def apply(
        ...
        """
        parsed_include_kinds: Sequence[SymbolKind] | None = (
            [SymbolKind(k) for k in include_kinds] if include_kinds else None
        )
        parsed_exclude_kinds: Sequence[SymbolKind] | None = (
            [SymbolKind(k) for k in exclude_kinds] if exclude_kinds else None
        )
        symbol_retriever = self.create_language_server_symbol_retriever()
...
class FindReferencingSymbolsTool(Tool):
    ...
    def apply(
        ...
        for ref in references_in_symbols:
            ref_dict = ref.symbol.to_dict(
                kind=True, location=True, depth=0, include_body=include_body
            )
            ref_dict = _sanitize_symbol_dict(ref_dict)
            if not include_body:
                ref_relative_path = ref.symbol.location.relative_path
                assert ref_relative_path is not None, (
                    f"Referencing symbol {ref.symbol.name} has no relative path, this is likely a bug."
                )
                content_around_ref = self.retrieve_content_around_line(
                    relative_file_path=ref_relative_path,
                    line=ref.line,
                    context_lines_before=1,
                    context_lines_after=1,
                )
                ref_dict["content_around_reference"] = (
                    content_around_ref.to_display_string()
                )
...
class InsertAfterSymbolTool(Tool):
    ...
    def apply(
        ...
        code_editor.insert_after_symbol(
            name_path, relative_file_path=relative_path, body=body
        )
        return SUCCESS_RESULT
