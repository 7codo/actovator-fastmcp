...
from app.constants import SERENA_MANAGED_DIR_IN_HOME, SERENA_MANAGED_DIR_NAME
...
class LanguageServerFactory:
    ...
    def create_language_server(self, language: Language) -> SolidLanguageServer:
        ...
        log.info(
            f"Creating language server instance for {self.project_root}, language={language}."
        )
...
class LanguageServerManager:
    ...
    @staticmethod
    def from_languages(
        languages: list[Language], factory: LanguageServerFactory
            ...
            try:
                with LogTime(f"Language server startup (language={language.value})"):
                    ...
                    if not language_server.is_running():
                        raise RuntimeError(
                            f"Failed to start the language server for language {language.value}"
                        )
                    with lock:
                        language_servers[language] = language_server
            except Exception as e:
                log.error(
                    f"Error starting language server for language {language.value}: {e}",
                    exc_info=e,
                )
                with lock:
    ...
    def _ensure_functional_ls(self, ls: SolidLanguageServer) -> SolidLanguageServer:
        if not ls.is_running():
            log.warning(
                f"Language server for language {ls.language} is not running; restarting ..."
            )
    ...
    def get_language_server(self, relative_path: str) -> SolidLanguageServer:
        ...
        if len(self._language_servers) > 1:
            for candidate in self._language_servers.values():
                if not candidate.is_ignored_path(
                    relative_path, ignore_unsupported_files=True
                ):
                    ls = candidate
                    break
        if ls is None:
    ...
    def _create_and_start_language_server(
        self, language: Language
    ) -> SolidLanguageServer:
        if self._language_server_factory is None:
            raise ValueError(
                f"No language server factory available to create language server for {language}"
            )
    ...
    def restart_language_server(self, language: Language) -> SolidLanguageServer:
        ...
        if language not in self._language_servers:
            raise ValueError(
                f"No language server for language {language.value} present; cannot restart"
            )
    ...
    def add_language_server(self, language: Language) -> SolidLanguageServer:
        ...
        if language in self._language_servers:
            raise ValueError(
                f"Language server for language {language.value} already present"
            )
        return self._create_and_start_language_server(language)

    def remove_language_server(
        self, language: Language, save_cache: bool = False
    ) -> None:
        """
        ...
        if language not in self._language_servers:
            raise ValueError(
                f"No language server for language {language.value} present; cannot remove"
            )
    ...
    def _stop_language_server(
        ls: SolidLanguageServer, save_cache: bool = False
    ) -> None:
        if ls.is_running():
            if save_cache:
                ls.save_cache()
