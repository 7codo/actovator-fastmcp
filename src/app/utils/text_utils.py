...
class MatchedConsecutiveLines:
    ...
    def to_display_string(self, include_line_numbers: bool = True) -> str:
        return "\n".join(
            [line.format_line(include_line_numbers) for line in self.lines]
        )
    ...
    def from_file_contents(
        ...
        text_lines.append(
            TextLine(
                line_number=line,
                line_content=line_contents[line],
        ...
        for match in compiled_pattern.finditer(content):
            ...
            matches.append(
                MatchedConsecutiveLines(
                    lines=context_lines, source_file_path=source_file_path
                )
            )
    else:
...
def search_files(
    ...
    for path in relative_file_paths:
        if include_patterns:
            if not any(glob_match(p, path) for p in include_patterns):
                log.debug(
                    f"Skipping {path}: does not match include pattern {paths_include_glob}"
                )
                continue
        ...
        if exclude_patterns:
            if any(glob_match(p, path) for p in exclude_patterns):
                log.debug(
                    f"Skipping {path}: matches exclude pattern {paths_exclude_glob}"
                )
                continue
    ...
    if skipped_file_error_tuples:
        log.debug(
            f"Failed to read {len(skipped_file_error_tuples)} files: {skipped_file_error_tuples}"
        )
