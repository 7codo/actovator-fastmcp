...
class LanguageServerSymbol(Symbol, ToStringMixin):
    ...
    def match_name_path(
        ...
        pattern_parts = (
            name_path.lstrip(name_path_sep).rstrip(name_path_sep).split(name_path_sep)
        )
    ...
    def _tostring_additional_entries(self) -> dict[str, Any]:
        return dict(
            name=self.name,
            kind=self.kind,
            num_children=len(self.symbol_root["children"]),
        )
    ...
    def location(self) -> LanguageServerSymbolLocation:
        ...
        """
        return LanguageServerSymbolLocation(
            relative_path=self.relative_path, line=self.line, column=self.column
        )
    ...
    def get_name_path_parts(self) -> list[str]:
        ...
        """
        ancestors_within_file = list(
            self.iter_ancestors(up_to_symbol_kind=SymbolKind.File)
        )
    ...
    def iter_ancestors(
        self, up_to_symbol_kind: SymbolKind | None = None
    ) -> Iterator[Self]:
        """
    ...
    def to_dict(
        ...
        if include_body:
            if self.body is None:
                log.warning(
                    "Requested body for symbol, but it is not present. The symbol might have been loaded with include_body=False."
                )
...
class ReferenceInLanguageServerSymbol(ToStringMixin):
    ...
    def from_lsp_reference(cls, reference: LSPReferenceInSymbol) -> Self:
        return cls(
            symbol=LanguageServerSymbol(reference.symbol),
            line=reference.line,
...
class LanguageServerSymbolRetriever:
    ...
    def find_by_name(
        ...
        for lang_server in self._ls_manager.iter_language_servers():
            symbol_roots = lang_server.request_full_symbol_tree(
    ...
    def find_referencing_symbols(
        ...
        """
        symbol_candidates = self.find_by_name(
            name_path, substring_matching=False, within_relative_path=relative_file_path
        )
        if len(symbol_candidates) == 0:
            log.warning(
                f"No symbol with name {name_path} found in file {relative_file_path}"
            )
            return []
    ...
    def find_referencing_symbols_by_location(
        ...
        if exclude_kinds is not None:
            references = [
                s for s in references if s.symbol["kind"] not in exclude_kinds
            ]

        return [
            ReferenceInLanguageServerSymbol.from_lsp_reference(r) for r in references
        ]
    ...
    def get_symbol_overview(
    ...
    ) -> dict[str, list[SymbolOverviewElement]]:
        lang_server = self.get_language_server(relative_path)
        ...
        for file_path, unified_symbols in path_to_unified_symbols.items():
            ...
            result[file_path] = [
                self.SymbolOverviewElement.from_symbol(LanguageServerSymbol(s))
