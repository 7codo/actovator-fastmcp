...
class Language(str, Enum):
    ...
    def get_ls_class(self) -> type["SolidLanguageServer"]:
        match self:
            case self.PYTHON:
                from app.solidlsp.language_servers.pyright_server import PyrightServer

                return PyrightServer
            case self.PYTHON_JEDI:
                from app.solidlsp.language_servers.jedi_server import JediServer

                return JediServer
            case self.JAVA:
                from app.solidlsp.language_servers.eclipse_jdtls import EclipseJDTLS

                return EclipseJDTLS
            case self.KOTLIN:
                from app.solidlsp.language_servers.kotlin_language_server import (
                    KotlinLanguageServer,
                )

                return KotlinLanguageServer
            case self.RUST:
                from app.solidlsp.language_servers.rust_analyzer import RustAnalyzer

                return RustAnalyzer
            case self.CSHARP:
                from app.solidlsp.language_servers.csharp_language_server import (
                    CSharpLanguageServer,
                )

                return CSharpLanguageServer
            case self.CSHARP_OMNISHARP:
                from app.solidlsp.language_servers.omnisharp import OmniSharp
            ...
            case self.TYPESCRIPT_VTS:
                from app.solidlsp.language_servers.vts_language_server import (
                    VtsLanguageServer,
                )

                return VtsLanguageServer
            case self.GO:
                from app.solidlsp.language_servers.gopls import Gopls

                return Gopls
            case self.RUBY:
                from app.solidlsp.language_servers.ruby_lsp import RubyLsp

                return RubyLsp
            case self.RUBY_SOLARGRAPH:
                from app.solidlsp.language_servers.solargraph import Solargraph

                return Solargraph
            case self.DART:
                from app.solidlsp.language_servers.dart_language_server import (
                    DartLanguageServer,
                )

                return DartLanguageServer
            case self.CPP:
                from app.solidlsp.language_servers.clangd_language_server import (
                    ClangdLanguageServer,
                )

                return ClangdLanguageServer
            case self.PHP:
                from app.solidlsp.language_servers.intelephense import Intelephense

                return Intelephense
            case self.PERL:
                from app.solidlsp.language_servers.perl_language_server import (
                    PerlLanguageServer,
                )

                return PerlLanguageServer
            case self.CLOJURE:
                from app.solidlsp.language_servers.clojure_lsp import ClojureLSP

                return ClojureLSP
            case self.ELIXIR:
                from app.solidlsp.language_servers.elixir_tools.elixir_tools import (
                    ElixirTools,
                )

                return ElixirTools
            case self.ELM:
                from app.solidlsp.language_servers.elm_language_server import (
                    ElmLanguageServer,
                )

                return ElmLanguageServer
            case self.TERRAFORM:
                from app.solidlsp.language_servers.terraform_ls import TerraformLS

                return TerraformLS
            case self.SWIFT:
                from app.solidlsp.language_servers.sourcekit_lsp import SourceKitLSP

                return SourceKitLSP
            case self.BASH:
                from app.solidlsp.language_servers.bash_language_server import (
                    BashLanguageServer,
                )

                return BashLanguageServer
            case self.ZIG:
                from app.solidlsp.language_servers.zls import ZigLanguageServer

                return ZigLanguageServer
            case self.NIX:
                from app.solidlsp.language_servers.nixd_ls import NixLanguageServer

                return NixLanguageServer
            case self.LUA:
                from app.solidlsp.language_servers.lua_ls import LuaLanguageServer

                return LuaLanguageServer
            case self.ERLANG:
                from app.solidlsp.language_servers.erlang_language_server import (
                    ErlangLanguageServer,
                )

                return ErlangLanguageServer
            case self.AL:
                from app.solidlsp.language_servers.al_language_server import (
                    ALLanguageServer,
                )

                return ALLanguageServer
            case self.REGO:
                from app.solidlsp.language_servers.regal_server import (
                    RegalLanguageServer,
                )

                return RegalLanguageServer
            case self.MARKDOWN:
                from app.solidlsp.language_servers.marksman import Marksman

                return Marksman
            case self.R:
                from app.solidlsp.language_servers.r_language_server import (
                    RLanguageServer,
                )

                return RLanguageServer
            case self.SCALA:
                from app.solidlsp.language_servers.scala_language_server import (
                    ScalaLanguageServer,
                )

                return ScalaLanguageServer
            case self.JULIA:
                from app.solidlsp.language_servers.julia_server import (
                    JuliaLanguageServer,
                )

                return JuliaLanguageServer
            case self.FORTRAN:
                from app.solidlsp.language_servers.fortran_language_server import (
                    FortranLanguageServer,
                )

                return FortranLanguageServer
            case self.HASKELL:
                from app.solidlsp.language_servers.haskell_language_server import (
