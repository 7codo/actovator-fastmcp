...
from typing import Union, cast, Self

import pathspec

from app.utils.text_utils import MatchedConsecutiveLines
from app.utils.file_system import match_path
from app.solidlsp import ls_types
from app.solidlsp.ls_config import Language, LanguageServerConfig
from app.solidlsp.ls_exceptions import SolidLSPException
from app.solidlsp.ls_handler import SolidLanguageServerHandler
from app.solidlsp.ls_logger import LanguageServerLogger
from app.solidlsp.ls_types import UnifiedSymbolInformation
from app.solidlsp.ls_utils import FileUtils, PathUtils, TextUtils
from app.solidlsp.lsp_protocol_handler import lsp_types
from app.solidlsp.lsp_protocol_handler import lsp_types as LSPTypes
from app.solidlsp.lsp_protocol_handler.lsp_constants import LSPConstants
from app.solidlsp.lsp_protocol_handler.lsp_types import (
    Definition,
    DefinitionParams,
    LocationLink,
    SymbolKind,
)
from app.solidlsp.lsp_protocol_handler.server import (
    LSPError,
    ProcessLaunchInfo,
    StringDict,
)
from app.solidlsp.settings import SolidLSPSettings
...
class SolidLanguageServer(ABC):
    ...
    def __init__(
        ...
        if config.trace_lsp_communication:
            ...
            def logging_fn(source: str, target: str, msg: StringDict | str):
                self.logger.log(
                    f"LSP: {source} -> {target}: {msg!s}", self.logger.logger.level
                )
        ...
        self.logger.log(
            f"Processing {len(processed_patterns)} ignored paths from the config",
            logging.DEBUG,
        )
    ...
    def _shutdown(self, timeout: float = 5.0):
        ...
        if not self.server.is_running():
            self.logger.log(
                "Server process not running, skipping shutdown.", logging.DEBUG
            )
            return

        self.logger.log(
            f"Initiating final robust shutdown with a {timeout}s timeout...",
            logging.INFO,
        )
        ...
        try:
            ...
            if shutdown_thread.is_alive():
                self.logger.log(
                    "LSP shutdown request timed out, proceeding to terminate...",
                    logging.DEBUG,
                )
            else:
        ...
        try:
            self.logger.log(
                f"Waiting for process {process.pid} to terminate...", logging.DEBUG
            )
            ...
            try:
                ...
                self.logger.log(
                    f"Process {process.pid} could not be killed within timeout.",
                    logging.ERROR,
                )
        except Exception as e:
    ...
    def request_references(
        ...
        for item in response:
            ...
            if self.is_ignored_path(str(rel_path)):
                self.logger.log(
                    f"Ignoring reference in {rel_path} since it should be ignored",
                    logging.DEBUG,
                )
                continue
    ...
    def request_document_symbols(
            ...
            with self._cache_lock:
                ...
                if file_hash_and_result is not None:
                    ...
                    if file_hash == file_data.content_hash:
                        self.logger.log(
                            f"Returning cached document symbols for {relative_file_path}",
                            logging.DEBUG,
                        )
                        return result
                    else:
                        self.logger.log(
                            f"Content for {relative_file_path} has changed. Will overwrite in-memory cache",
                            logging.DEBUG,
                        )
                else:
                    self.logger.log(
                        f"No cache hit for symbols with {include_body=} in {relative_file_path}",
                        logging.DEBUG,
                    )
        ...
        for root_item in response:
            ...
            if LSPConstants.CHILDREN in root_symbol:
                ...
                flat_all_symbol_list.extend(
                    visit_tree_nodes_and_build_tree_repr(root_symbol)
                )
            else:
                flat_all_symbol_list.append(
                    ls_types.UnifiedSymbolInformation(**root_symbol)
                )

        result = flat_all_symbol_list, root_nodes
        self.logger.log(
            f"Caching document symbols for {relative_file_path}", logging.DEBUG
        )
        with self._cache_lock:
    ...
    def request_full_symbol_tree(
        ...
        if within_relative_path is not None:
            ...
            if os.path.isfile(within_abs_path):
                if self.is_ignored_path(within_relative_path):
                    ...
                    _, root_nodes = self.request_document_symbols(
                        within_relative_path, include_body=include_body
                    )
                    return root_nodes
        ...
        def process_directory(
            ...
            if self.is_ignored_path(
                ...
                self.logger.log(
                    f"Skipping directory: {rel_dir_path}\n(because it should be ignored)",
                    logging.DEBUG,
                )
                return []
            ...
            for contained_dir_or_file_name in contained_dir_or_file_names:
                ...
                if self.is_ignored_path(contained_dir_or_file_rel_path):
                    self.logger.log(
                        f"Skipping item: {contained_dir_or_file_rel_path}\n(because it should be ignored)",
                        logging.DEBUG,
                    )
                    continue
                ...
                if os.path.isdir(contained_dir_or_file_abs_path):
                    ...
                    _, file_root_nodes = self.request_document_symbols(
                        contained_dir_or_file_rel_path, include_body=include_body
                    )
                    ...
                    with self.open_file(file_rel_path) as file_data:
                        fileRange = self._get_range_from_file_content(
                            file_data.contents
                        )
                    file_symbol = ls_types.UnifiedSymbolInformation(  # type: ignore
                        ...
                        location=ls_types.Location(
                            uri=str(
                                pathlib.Path(contained_dir_or_file_abs_path).as_uri()
                            ),
                            range=fileRange,
    ...
    def retrieve_symbol_body(
        ...
        symbol_file = self.retrieve_full_file_content(
    ...
    def request_containing_symbol(
        ...
        var_containers = [
            s for s in symbols if s["kind"] == ls_types.SymbolKind.Variable
        ]
    ...
    def save_cache(self):
        with self._cache_lock:
            if not self._cache_has_changed:
                self.logger.log(
                    "No changes to document symbols cache, skipping save", logging.DEBUG
                )
                return

            self.logger.log(
                f"Saving updated document symbols cache to {self.cache_path}",
                logging.INFO,
            )
    ...
    def load_cache(self):
        ...
        with self._cache_lock:
            self.logger.log(
                f"Loading document symbols cache from {self.cache_path}", logging.INFO
            )
            try:
                with open(self.cache_path, "rb") as f:
                    self._document_symbols_cache = pickle.load(f)
                self.logger.log(
                    f"Loaded {len(self._document_symbols_cache)} document symbols from cache.",
                    logging.INFO,
                )
            except Exception as e:
    ...
    def stop(self, shutdown_timeout: float = 2.0) -> None:
        ...
        try:
            ...
            self.logger.log(
                f"Exception while shutting down language server: {e}", logging.WARNING
            )
