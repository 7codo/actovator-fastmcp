...
class SourceKitLSP(SolidLanguageServer):
    ...
    def __init__(
        ...
        logger.log(
            f"Starting sourcekit lsp with version: {sourcekit_version}", logging.INFO
        )
    ...
    def _start_server(self):
        ...
        self.logger.log(
            f"SourceKit LSP capabilities: {list(capabilities.keys())}", logging.INFO
        )
    ...
    def request_references(
        ...
        if os.getenv("CI") and not references:
            self.logger.log(
                "No references found in CI - retrying after additional 5s delay",
                logging.INFO,
            )
            time.sleep(5)
            references = super().request_references(relative_file_path, line, column)
