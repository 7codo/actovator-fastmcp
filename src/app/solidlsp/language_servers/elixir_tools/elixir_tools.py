...
from app.solidlsp.ls_utils import FileUtils, PlatformId, PlatformUtils
...
class ElixirTools(SolidLanguageServer):
        ...
        for item in raw_response:
            if isinstance(item, dict) and "uri" in item:
                ...
                if self._is_next_ls_internal_file(abs_path):
                    self.logger.log(
                        f"Filtering out Next LS internal file: {abs_path}",
                        logging.DEBUG,
                    )
                    continue
    ...
    def _setup_runtime_dependencies(
        ...
        if not os.path.exists(executable_path):
            logger.log(
                f"Downloading Next LS binary from {dependency.url}", logging.INFO
            )
        ...
        assert os.path.exists(executable_path), (
            f"Next LS executable not found at {executable_path}"
        )
    ...
    def __init__(
        ...
        nextls_executable_path = self._setup_runtime_dependencies(
        ...
        super().__init__(
            ...
            ProcessLaunchInfo(
                cmd=f'"{nextls_executable_path}" --stdio', cwd=repository_root_path
            ),
            "elixir",
            solidlsp_settings,
        )
    ...
    def _start_server(self):
        ...
        def window_log_message(msg):
            ...
            if "Runtime for folder" in message_text and "is ready..." in message_text:
                self.logger.log(
                    "Next LS runtime is ready based on official log message",
                    logging.INFO,
                )
        ...
        def check_server_ready(params):
            ...
            if value.get("kind") == "end":
                ...
                if "has initialized!" in message:
                    self.logger.log(
                        "Next LS initialization progress completed", logging.INFO
                    )
        ...
        self.logger.log(
            f"Next LS capabilities: {list(init_response['capabilities'].keys())}",
            logging.INFO,
        )
        ...
        if "completionProvider" not in init_response["capabilities"]:
            self.logger.log(
                "Warning: completionProvider not available in initial capabilities",
                logging.WARNING,
            )
        if "definitionProvider" not in init_response["capabilities"]:
            self.logger.log(
                "Warning: definitionProvider not available in initial capabilities",
                logging.WARNING,
            )
        ...
        ready_timeout = 180.0
        self.logger.log(
            f"Waiting up to {ready_timeout} seconds for Next LS runtime readiness...",
            logging.INFO,
        )
        ...
        if self.server_ready.wait(timeout=ready_timeout):
            ...
            settling_time = 120.0
            self.logger.log(
                f"Allowing {settling_time} seconds for Next LS background indexing to complete...",
                logging.INFO,
            )
