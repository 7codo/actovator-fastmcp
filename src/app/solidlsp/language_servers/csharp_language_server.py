...
from app.solidlsp.lsp_protocol_handler.lsp_types import InitializeParams
from app.solidlsp.lsp_protocol_handler.server import ProcessLaunchInfo
from app.solidlsp.settings import SolidLSPSettings
from app.solidlsp.util.zip import SafeZipExtractor
...
class CSharpLanguageServer(SolidLanguageServer):
    ...
    def __init__(
        ...
        dotnet_path, language_server_path = self._ensure_server_installed(
            logger, config, solidlsp_settings
        )
        ...
        if solution_or_project:
            logger.log(
                f"Found solution/project file: {solution_or_project}", logging.INFO
            )
        else:
            logger.log(
    ...
    def _ensure_server_installed(
        ...
        dotnet_path = CSharpLanguageServer._ensure_dotnet_runtime(
            logger, dotnet_runtime_dep, solidlsp_settings
        )
        server_dll_path = CSharpLanguageServer._ensure_language_server(
            logger, lang_server_dep, solidlsp_settings
        )
    ...
    def _ensure_dotnet_runtime(
        cls,
        logger: LanguageServerLogger,
        dotnet_runtime_dep: RuntimeDependency,
        solidlsp_settings: SolidLSPSettings,
    ) -> str:
    ...
    def _ensure_language_server(
        cls,
        logger: LanguageServerLogger,
        lang_server_dep: RuntimeDependency,
        solidlsp_settings: SolidLSPSettings,
    ) -> str:
        ...
        logger.log(
            f"Downloading {package_name} version {package_version}...", logging.INFO
        )
        package_path = cls._download_nuget_package_direct(
            logger, package_name, package_version, solidlsp_settings
        )
    ...
    def _ensure_dotnet_runtime_from_config(
        ...
        if custom_dotnet_runtime_url is not None:
            logger.log(
                f"Using custom .NET runtime url: {custom_dotnet_runtime_url}",
                logging.INFO,
            )
        ...
        try:
            ...
            logger.log(
                f"Successfully installed .NET 9 runtime to {dotnet_exe}", logging.INFO
            )
            return str(dotnet_exe)
    ...
    def _start_server(self):
        ...
        def handle_progress(params):
            ...
            if kind == "begin":
                ...
                if percentage is not None:
                    ...
                    self.logger.log(
                        f"Progress [{token}]: {title} - {message}", logging.INFO
                    )
                ...
                if percentage is not None:
                    self.logger.log(
                        f"Progress [{token}]: {message} ({percentage}%)", logging.INFO
                    )
                elif message:
        ...
        self.logger.log(
            "Starting Microsoft.CodeAnalysis.LanguageServer process", logging.INFO
        )
        ...
        try:
            ...
            self.logger.log(
                f"Received initialize response: {init_response}", logging.DEBUG
            )
        except Exception as e:
            raise SolidLSPException(
    ...
    def _force_pull_diagnostics(self, init_response: dict) -> None:
        ...
        if isinstance(diagnostic_provider, dict):
            ...
            self.logger.log(
                "Applied diagnostic capabilities hack for better C# diagnostics",
                logging.DEBUG,
            )
