...
class RustAnalyzer(SolidLanguageServer):
    ...
    def _ensure_rust_analyzer_installed():
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"Failed to install rust-analyzer via rustup: {result.stderr}"
            )
        ...
        if not path:
            raise RuntimeError(
                "rust-analyzer installation succeeded but binary not found in PATH"
            )
    ...
    def __init__(
        ...
        logger.log(
            f"Using rust-analyzer at: {rustanalyzer_executable_path}", logging.INFO
        )
