...
class ErlangLanguageServer(SolidLanguageServer):
    ...
    def _start_server(self):
        ...
        def window_log_message(msg):
            ...
            for signal in readiness_signals:
                if signal.lower() in message_lower:
                    self.logger.log(
                        f"Erlang LS readiness signal detected: {message_text}",
                        logging.INFO,
                    )
        ...
        def check_server_ready(params):
            ...
            if value.get("kind") == "end":
                ...
                if any(
                    ...
                    self.logger.log(
                        "Erlang LS initialization progress completed", logging.INFO
                    )
        ...
        if self.server_ready.wait(timeout=ready_timeout):
            self.logger.log(
                "Erlang LS is ready and available for requests", logging.INFO
            )
            ...
            self.logger.log(
                "Basic Erlang LS initialization period complete", logging.INFO
            )
