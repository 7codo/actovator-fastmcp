...
from app.solidlsp.language_servers.common import quote_windows_path
from app.solidlsp.ls import SolidLanguageServer
from app.solidlsp.ls_config import LanguageServerConfig
...
from app.solidlsp.lsp_protocol_handler.server import ProcessLaunchInfo
...
class ALLanguageServer(SolidLanguageServer):
    ...
    def _download_al_extension(
        cls, logger: LanguageServerLogger, url: str, target_dir: str
    ) -> bool:
        """
        ...
        try:
            ...
            logger.log(
                f"Downloading {total_size / 1024 / 1024:.1f} MB...", logging.INFO
            )
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if (
                            ...
                            logger.log(
                                f"Download progress: {progress:.1f}%", logging.INFO
                            )
    ...
    def _setup_runtime_dependencies(
        ...
        if extension_path is None:
            logger.log(
                "AL extension not found on disk, attempting to download...",
                logging.INFO,
            )
            extension_path = cls._download_and_install_al_extension(
                logger, solidlsp_settings
            )
    ...
    @classmethod
    def _find_al_extension(
        cls, logger: LanguageServerLogger, solidlsp_settings: SolidLSPSettings
    ) -> str | None:
        """
        ...
        if env_path and os.path.exists(env_path):
            logger.log(
                f"Found AL extension via AL_EXTENSION_PATH: {env_path}", logging.DEBUG
            )
            return env_path
        elif env_path:
            logger.log(
                f"AL_EXTENSION_PATH set but directory not found: {env_path}",
                logging.WARNING,
            )
        ...
        if os.path.exists(default_path):
            logger.log(
                f"Found AL extension in default location: {default_path}", logging.DEBUG
            )
            return default_path
    ...
    def _download_and_install_al_extension(
        ...
        if cls._download_al_extension(logger, url, al_extension_dir):
            ...
            if os.path.exists(extension_path):
                logger.log(
                    "AL extension downloaded and installed successfully", logging.INFO
                )
            ...
            logger.log(
                "Failed to download AL extension from marketplace", logging.ERROR
            )
    ...
    def _prepare_executable(
        cls, executable_path: str, system: str, logger: LanguageServerLogger
    ) -> str:
        """
        ...
        logger.log(
            f"Using AL Language Server executable: {executable_path}", logging.INFO
        )
    ...
    def _start_server(self):
        ...
        def publish_diagnostics(params):
            ...
            self.logger.log(
                f"AL LSP: Diagnostics for {uri}: {len(diagnostics)} issues",
                logging.DEBUG,
            )
    ...
    def _post_initialize_al_workspace(self) -> None:
        ...
        try:
            ...
            self.logger.log(
                f"Failed to set active workspace (non-critical): {e}", logging.DEBUG
            )
    ...
    def request_full_symbol_tree(
        ...
        """
        self.logger.log(
            "AL: Starting request_full_symbol_tree with file opening", logging.DEBUG
        )
        ...
        for file_path, relative_path in al_files:
            try:
                ...
                self.logger.log(
                    f"AL: Getting symbols for {relative_path}", logging.DEBUG
                )
                all_syms, root_syms = self.request_document_symbols(
                    relative_path, include_body=include_body
                )
                ...
                if root_syms:
                    ...
                    all_file_symbols.append(file_symbol)
                    self.logger.log(
                        f"AL: Added {len(all_syms)} symbols from {relative_path}",
                        logging.DEBUG,
                    )

            except Exception as e:
                self.logger.log(
                    f"AL: Failed to get symbols for {relative_path}: {e}",
                    logging.WARNING,
                )

        if all_file_symbols:
            self.logger.log(
                f"AL: Returning symbols from {len(all_file_symbols)} files",
                logging.DEBUG,
            )
    ...
    def check_project_loaded(self) -> bool:
        ...
        if not hasattr(self, "server") or not self.server_started:
            self.logger.log(
                "Cannot check project load - server not started", logging.DEBUG
            )
            return False
    ...
    def _wait_for_project_load(self, timeout: int = 3) -> bool:
        ...
        self.logger.log(
            f"Checking AL project load status (timeout: {timeout}s)...", logging.DEBUG
        )
        ...
        while time.time() - start_time < timeout:
            if self.check_project_loaded():
                ...
                self.logger.log(
                    f"AL project fully loaded after {elapsed:.1f}s", logging.INFO
                )
                return True
            time.sleep(0.5)

        self.logger.log(
            f"Project load check timed out after {timeout}s (non-critical)",
            logging.DEBUG,
        )
        return False
    ...
    def set_active_workspace(self, workspace_uri: str | None = None) -> None:
        ...
        if not hasattr(self, "server") or not self.server_started:
            self.logger.log(
                "Cannot set active workspace - server not started", logging.DEBUG
            )
            return
