...
from app.solidlsp.util.subprocess_util import subprocess_kwargs
...
class RuntimeDependencyCollection:
    ...
    def _install_from_url(
        dep: RuntimeDependency, logger: LanguageServerLogger, target_dir: str
    ) -> None:
        ...
        if dep.archive_type in ("gz", "binary") and dep.binary_name:
            ...
            FileUtils.download_and_extract_archive(
                logger, dep.url, dest, dep.archive_type
            )
        else:
            FileUtils.download_and_extract_archive(
                logger, dep.url, target_dir, dep.archive_type or "zip"
            )
