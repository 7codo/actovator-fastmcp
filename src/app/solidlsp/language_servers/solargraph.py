...
class Solargraph(SolidLanguageServer):
        ...
        if is_bundler_project:
            ...
            if os.path.exists(gemfile_lock_path):
                try:
                    ...
                    logger.log(
                        f"Warning: Could not read Gemfile.lock: {e}", logging.WARNING
                    )
    ...
    def _start_server(self):
        ...
        self.logger.log(
            "Waiting for Solargraph to complete initial workspace analysis...",
            logging.INFO,
        )
        if self.analysis_complete.wait(timeout=60.0):
            self.logger.log(
                "Solargraph initial analysis complete, server ready", logging.INFO
            )
        else:
            self.logger.log(
