...
class RubyLsp(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(
        ...
        if use_rbenv:
            ...
            logger.log(
                f"Using rbenv-managed Ruby (found {ruby_version_file})", logging.INFO
            )
        else:
            ruby_cmd = ["ruby"]
            bundle_cmd = ["bundle"]
            if os.path.exists(ruby_version_file):
                ...
                logger.log(
                    "No .ruby-version file found, using system Ruby", logging.INFO
                )
            ...
            if not bundle_path:
                ...
                if os.path.exists(gemfile_lock_path):
                    try:
                        ...
                        logger.log(
                            f"Warning: Could not read Gemfile.lock: {e}",
                            logging.WARNING,
                        )
        ...
        logger.log(
            "ruby-lsp not found, attempting to install globally...", logging.INFO
        )
        try:
            subprocess.run(
    ...
    def _start_server(self) -> None:
        ...
        def register_capability_handler(params: dict) -> None:
            ...
            for registration in params["registrations"]:
                self.logger.log(
                    f"Registered capability: {registration['method']}", logging.INFO
                )
            return
        ...
        def progress_handler(params: dict) -> None:
            ...
            if "value" in params:
                ...
                if value.get("kind") == "end":
                    self.logger.log(
                        "ruby-lsp indexing complete ($/progress end)", logging.INFO
                    )
                    self.analysis_complete.set()
                    self.completions_available.set()
                elif value.get("kind") == "begin":
                    self.logger.log(
                        "ruby-lsp indexing started ($/progress begin)", logging.INFO
                    )
                elif "percentage" in value:
                    percentage = value.get("percentage", 0)
                    self.logger.log(
                        f"ruby-lsp indexing progress: {percentage}%", logging.DEBUG
                    )
            # Handle direct progress format (fallback)
                ...
                if isinstance(token, str) and "indexing" in token.lower():
                    ...
                    if value.get("kind") == "end" or value.get("percentage") == 100:
                        self.logger.log(
                            "ruby-lsp indexing complete (token progress)", logging.INFO
                        )
                        self.analysis_complete.set()
                        self.completions_available.set()
        ...
        self.logger.log(
            f"Sending init params: {json.dumps(initialize_params, indent=4)}",
            logging.INFO,
        )
        ...
        self.logger.log(
            "Waiting for ruby-lsp to complete initial indexing...", logging.INFO
        )
        if self.analysis_complete.wait(timeout=30.0):
            self.logger.log(
                "ruby-lsp initial indexing complete, server ready", logging.INFO
            )
        else:
            self.logger.log(
