...
from app.solidlsp.language_servers.common import (
    RuntimeDependency,
...
class BashLanguageServer(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(
        ...
        if not os.path.exists(bash_executable_path):
            ...
            logger.log(
                "Bash language server dependencies installed successfully", logging.INFO
            )
    ...
    def _start_server(self):
        ...
        def window_log_message(msg):
            ...
            if (
                ...
                self.logger.log(
                    "Bash language server analysis signals detected", logging.INFO
                )
        ...
        self.logger.log(
            f"Received initialize response from bash server: {init_response}",
            logging.DEBUG,
        )
        ...
        if "documentSymbolProvider" in init_response["capabilities"]:
            ...
            self.logger.log(
                "Warning: Bash server does not report document symbol support",
                logging.WARNING,
            )
        ...
        if not self.server_ready.wait(timeout=3.0):
            ...
            self.logger.log(
                "Timeout waiting for bash server ready signal, proceeding anyway",
                logging.WARNING,
            )
            self.server_ready.set()
            self.completions_available.set()
    ...
    def request_document_symbols(
        ...
        """
        self.logger.log(
            f"Requesting document symbols via LSP for {relative_file_path}",
            logging.DEBUG,
        )

        # Use the standard LSP approach - bash-language-server handles all function syntaxes correctly
        all_symbols, root_symbols = super().request_document_symbols(
            relative_file_path, include_body
        )
