...
from app.solidlsp.ls_utils import PathUtils, PlatformUtils
...
class TerraformLS(SolidLanguageServer):
    ...
    def _ensure_tf_command_available(logger: LanguageServerLogger):
        ...
        if terraform_cmd is not None:
            logger.log(
                f"Found terraform via shutil.which: {terraform_cmd}", logging.DEBUG
            )
            return
        ...
        if not terraform_cmd:
            ...
            if terraform_cli_path:
                logger.log(
                    f"Trying TERRAFORM_CLI_PATH: {terraform_cli_path}", logging.DEBUG
                )
